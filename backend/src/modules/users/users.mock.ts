export const MOCK_REQRES_USERS = [
...
export const getMockUsersPage = (page: number, perPage = 6) => {
  const start = (page - 1) * perPage
  const data = MOCK_REQRES_USERS.slice(start, start + perPage)
  return {
    page,
    per_page: perPage,
    total: MOCK_REQRES_USERS.length,
    total_pages: Math.ceil(MOCK_REQRES_USERS.length / perPage),
    data,
  }
}

export const getMockUserById = (id: number) => {
  return MOCK_REQRES_USERS.find((u) => u.id === id) ?? null
}
